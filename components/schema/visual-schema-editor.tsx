...
function ColumnItem({ column, isSelected, onClick }: ColumnItemProps) {
  ...
  const isPrimaryKey = column.constraints?.some(c => c.type.includes('PRIMARY KEY'));
  const isForeignKey = column.constraints?.some(c => c.type.includes('FOREIGN KEY'));
  ...
  return (
    <div
      className={cn(
        ...
        isPrimaryKey && 'bg-yellow-50 border-yellow-200',
        isForeignKey && 'bg-blue-50 border-blue-200'
      )}
      onClick={onClick}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-1 text-muted-foreground">
          ...
          {column.length && <span>({column.length})</span>}
          {!isNullable && <span className="text-red-600">NOT NULL</span>}
        </div>
      </div>
    </div>
  );
            ...
            <div className="space-y-1">
              {table.indexes.map((index) => (
                <div key={index.id} className="flex items-center gap-2 text-xs">
                  <Badge variant={index.unique ? "default" : "outline"} className="text-xs">
...
function PropertiesPanel({
  ...
  const [editingTable, setEditingTable] = useState<Table | undefined>(selectedTable);
  const [editingColumn, setEditingColumn] = useState<Column | undefined>(selectedColumn);
    ...
    return (
      <Card className="w-80">
        <CardContent className="text-center py-8">
          ...
          <p className="text-muted-foreground">Select a table or column to edit properties</p>
        </CardContent>
      </Card>
    );
  }
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input
              ...
              onChange={(e) => setEditingTable(prev => prev ? { ...prev, name: e.target.value } : undefined)}
            />
          </div>
          ...
          <div>
            ...
            <Input
              ...
              onChange={(e) => setEditingTable(prev => prev ? { ...prev, comment: e.target.value } : undefined)}
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input
              ...
              onChange={(e) => setEditingColumn(prev => prev ? { ...prev, name: e.target.value } : undefined)}
            />
          </div>
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(value) => setEditingColumn(prev => prev ? { ...prev, type: value as any } : undefined)}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          {(editingColumn.type === 'VARCHAR' || editingColumn.type === 'CHAR') && (
            <div>
...
function VisualSchemaEditorComponent({
  ...
  const addNewTable = useCallback(() => {
    ...
    const newTable: Table = {
      ...
      comment: 'New table',
      position: { x: 100 + schema.tables.length * 50, y: 100 + schema.tables.length * 50 },
      columns: [
        {
          id: generateId(),
          name: 'id',
      ...
      <div className="flex-1 relative">
        ...
        >
          ...
          <Panel position="top-left" className="space-x-2">
            {!readonly && (
              <>
                ...
                <Button 
                  ...
                  className={cn(
                    'gap-2',
                    isCreatingRelationship && 'bg-primary text-primary-foreground'
                  )}
                >
            ...
            <Button size="sm" variant="outline" onClick={autoLayout} className="gap-2">
              <Grid className="h-4 w-4" />
              Auto Layout
            </Button>
            
            <Button size="sm" variant="outline" onClick={() => fitView()} className="gap-2">
            ...
            <Card className="min-w-[200px]">
              ...
              <CardContent className="text-xs space-y-1">
                ...
                <div className="flex justify-between">
                  ...
                  <Badge variant="outline">{schema.relationships?.length || 0}</Badge>
                </div>
                <div className="flex justify-between">
                  <span>Columns:</span>
              ...
              <Card>
                <CardContent className="p-3">
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>Click and drag between table nodes to create relationships</span>
                    <Button 
                      size="sm" 
                      variant="outline" 
