...
import { generateMigrations, type MigrationOptions } from '@/lib/supabase/migration-formatter';
import type { DatabaseSchema } from '@/types/schema.types';
import type { SupabaseProject, DeploymentResult } from '@/lib/supabase/management';
...
export function MigrationDeployer({
  ...
  const [deploymentOptions, setDeploymentOptions] = useState<Partial<MigrationOptions>>({
    includeRLS: true,
    includeIndexes: true,
  ...
  const deploySchema = useCallback(async () => {
    ...
    try {
      ...
      const verificationResult = await managementClient.testConnection(project.id);
      ...
      const runningStep = deploymentSteps.find(step => step.status === 'running');
      if (runningStep) {
  ...
  }, [project, isDeploying, schema, deploymentOptions, oauth, updateStepStatus, deploymentSteps, onDeploymentComplete]);
  ...
  const getOverallProgress = (): number => {
    const completedSteps = deploymentSteps.filter(step => 
      step.status === 'completed' || step.status === 'failed'
    ).length;
    return deploymentSteps.length > 0 ? (completedSteps / deploymentSteps.length) * 100 : 0;
  };

  const hasFailures = deploymentSteps.some(step => step.status === 'failed');
  const isComplete = deploymentSteps.length > 0 && deploymentSteps.every(step => 
    step.status === 'completed' || step.status === 'failed'
  );
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <label key={key} className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={deploymentOptions[key as keyof MigrationOptions] as boolean}
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Badge variant={hasFailures ? 'destructive' : isComplete ? 'default' : 'secondary'}>
                {hasFailures ? 'Failed' : isComplete ? 'Completed' : 'In Progress'}
              </Badge>
            </div>
            {!isComplete && (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">
                      {hasFailures ? 'Deployment Failed' : 'Deployment Successful'}
                    </span>
                  </div>
