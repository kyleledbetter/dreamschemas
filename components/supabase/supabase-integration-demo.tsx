...
import type { SupabaseProject, CreateProjectResponse, DeploymentResult } from '@/lib/supabase/management';
  ...
  const [schema] = useState<DatabaseSchema>(() => ({
    ...
    tables: [
      {
        ...
        columns: [
          ...
          },
          {
            ...
            defaultValue: "'free'",
            constraints: [
              { type: 'CHECK', value: "subscription_tier IN ('free', 'premium', 'enterprise')" }
            ],
      ...
      },
      {
        ...
        columns: [
          ...
          },
          {
            ...
            defaultValue: "'member'",
            constraints: [
              { type: 'CHECK', value: "role IN ('owner', 'admin', 'member', 'viewer')" }
            ],
      ...
      },
      {
        ...
        columns: [
          ...
          },
          {
            ...
            defaultValue: "'active'",
            constraints: [
              { type: 'CHECK', value: "status IN ('active', 'archived', 'deleted')" }
            ]
          },
          {
    ...
    rlsPolicies: [
      ...
      },
      {
        ...
        command: 'SELECT',
        using: 'user_id = auth.uid() OR EXISTS (SELECT 1 FROM workspaces WHERE workspaces.id = workspace_members.workspace_id AND workspaces.owner_id = auth.uid())',
        roles: ['authenticated']
      },
      {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Complete workflow from OAuth authentication to project management and schema deployment.
            This demo shows the full integration with Supabase&apos;s Management API.
          </p>
        </div>
        ...
        <div className="mt-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 bg-muted/30 rounded-lg">
            <div className="text-center">
              <p className="text-2xl font-bold text-primary">{schema.tables.length}</p>
              <p className="text-sm text-muted-foreground">Tables</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-blue-600">{schema.relationships.length}</p>
              <p className="text-sm text-muted-foreground">Relationships</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-green-600">{schema.rlsPolicies.length}</p>
              <p className="text-sm text-muted-foreground">RLS Policies</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-purple-600">
                {schema.tables.reduce((acc, table) => acc + table.indexes.length, 0)}
              </p>
